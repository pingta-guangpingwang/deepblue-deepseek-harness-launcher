...
!macro DownloadMirror URL LABEL
  ${If} $DownloadStatus != "OK"
    ...
    inetc::get /USERAGENT "DeepBlue-DeepSeek-Harness-Bootstrap/${SHELL_VERSION}" /RESUME "" "${URL}" "$PLUGINSDIR\launcher-shell.7z" /END
    Pop $DownloadStatus
  ${EndIf}
!macroend
...
Section "安装启动器" SEC_MAIN
  ...
  ${Else}
    ...
    DetailPrint "正在检测 Gitee 国内镜像；不可用时依次切换 GitHub 和 OSS"
  ...
  ${If} $DownloadStatus != "OK"
    MessageBox MB_ICONSTOP "Gitee、GitHub 与 OSS 三条 UI 壳线路均下载失败（$DownloadStatus）。请检查网络后重试，或使用百度网盘完整离线包。"
    Abort
  ${EndIf}
