...
const assetRoots = [
  process.env.PET_STORE_ASSET_ROOT,
  path.join(root, 'pet-store'),
  path.resolve(root, '..', 'deepseek-harness-pets'),
].filter(Boolean)

async function isAssetRoot(candidate) {
  try {
    await Promise.all([
      stat(path.join(candidate, 'assets')),
      stat(path.join(candidate, 'thumbnails')),
    ])
    return true
  } catch {
    return false
...
let assetRoot
for (const candidate of assetRoots) {
  if (await isAssetRoot(candidate)) {
    assetRoot = candidate
    break
  }
}
...
if (!assetRoot) {
  throw new Error('Pet media repository is unavailable. Clone deepseek-harness-pets next to the launcher or set PET_STORE_ASSET_ROOT.')
}
...
console.log(`Verified ${manifest.payload.items.length} signed pet catalog entries from ${assetRoot}.`)
