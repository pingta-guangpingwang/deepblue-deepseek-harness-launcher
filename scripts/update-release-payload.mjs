...
payload.launcher = {
  ...
  notes: [
    '皮肤商店新增默认关闭的外部来源页签：首批收录 7 个上游仓库、28 个横屏素材和 4 个循环动图，每张卡片明确显示作者、来源与许可证状态',
    '外部素材始终从原仓库下载，不转存到本站；下载前按签名目录校验固定字节数与 SHA-256，并在镜像失败后自动回退原地址',
    ...
    '每个签名模块下载前依次检测 Gitee、GitHub 和 OSS 三条真实渠道；当前渠道不可用、下载失败或校验失败时自动切换下一条',
    '在线安装器保持不到 1 MB：优先从 Gitee 获取界面壳，失败时依次改走 GitHub 和 OSS；Node、Harness 与包管理器同样按功能分模块安装',
