...
const pinnedNewHost = (payload, label) => {
  if (!expectedNewAgentHostVersion || !/^[a-f0-9]{64}$/.test(expectedNewAgentHostSha256 || '') || !Number.isSafeInteger(expectedNewAgentHostSize) || expectedNewAgentHostSize < 1 || !Number.isSafeInteger(expectedNewAgentHostUnpackedSize) || expectedNewAgentHostUnpackedSize < expectedNewAgentHostSize) throw new Error(`${label} requires a pinned new Agent Host version, digest and sizes`)
  const host = payload.runtimeModules.find((module) => module.id === 'agent-host')
  const artifact = host?.artifacts?.find((candidate) => candidate.platform === 'win32' && candidate.arch === 'x64')
  const expectedFileName = `agent-host-${expectedNewAgentHostVersion}-win-x64.tar.gz`
  ...
  if (host?.version !== expectedNewAgentHostVersion || host.required !== false || host.installWhen !== 'launcher' || !Array.isArray(host.dependencies) || host.dependencies.length !== 0 || host.probe !== undefined || !Array.isArray(host.artifacts) || host.artifacts.length !== 1 || JSON.stringify(Object.keys(host || {}).sort()) !== JSON.stringify(['artifacts', 'dependencies', 'id', 'installWhen', 'required', 'version']) || artifact?.platform !== 'win32' || artifact?.arch !== 'x64' || artifact?.format !== 'tar.gz' || artifact?.sha256 !== expectedNewAgentHostSha256 || artifact?.size !== expectedNewAgentHostSize || artifact?.unpackedSize !== expectedNewAgentHostUnpackedSize || JSON.stringify(Object.keys(artifact || {}).sort()) !== JSON.stringify(['arch', 'format', 'mirrors', 'platform', 'sha256', 'size', 'unpackedSize']) || JSON.stringify(mirrorIds) !== JSON.stringify(['oss', 'github']) || artifact.mirrors.some((mirror) => JSON.stringify(Object.keys(mirror).sort()) !== JSON.stringify(['id', 'url'])) || mirrorUrls.some((url) => url.protocol !== 'https:' || url.username || url.password || url.search || url.hash) || mirrorUrls[0]?.hostname !== hostname || mirrorUrls[1]?.hostname !== 'github.com' || decodedPaths[0] !== `/modules/${expectedFileName}` || decodedPaths[1] !== `/pingta-guangpingwang/deepblue-deepseek-harness-launcher/releases/download/agent-host-${expectedNewAgentHostVersion}/${expectedFileName}`) throw new Error(`${label} Agent Host does not match the pinned canonical release`)
...
const verifyPinnedMirrors = async (artifact) => {
  for (const mirror of artifact.mirrors) {
    const remote = await fetchBoundedBytes(mirror.url, mirror.id === 'github'
      ? { maxBytes: expectedNewAgentHostSize, allowedRedirectHosts: ['github.com', '.githubusercontent.com'], maxRedirects: 5, timeoutMs: 30_000 }
      : { maxBytes: expectedNewAgentHostSize, redirect: 'error', timeoutMs: 30_000 })
    ...
    if (mirror.id === 'oss' && finalHost !== hostname) throw new Error('Agent Host OSS mirror redirected unexpectedly')
    ...
    if (remote.bytes.length !== expectedNewAgentHostSize || createHash('sha256').update(remote.bytes).digest('hex') !== expectedNewAgentHostSha256) throw new Error(`${mirror.id} Agent Host mirror digest mismatch during manifest publication`)
  }
}

if (objectKey.startsWith('modules/')) {
  if (objectKey !== `modules/agent-host-${expectedNewAgentHostVersion}-win-x64.tar.gz` || sha256 !== expectedNewAgentHostSha256 || bytes.length !== expectedNewAgentHostSize) throw new Error('Agent Host object does not match the pinned new release version, digest and size')
...
if (objectKey === 'release-v2/launcher-manifest.json') {
  ...
  if (!expectedLauncherVersion || !expectedAgentHostVersion) throw new Error('Manifest publication requires pinned public Launcher and Agent Host versions')
  ...
  if (changed.length !== 1 || changed[0] !== 'agent-host') throw new Error(`Candidate manifest changes unexpected modules: ${changed.join(', ') || 'none'}`)
...
if (manifestPublication) {
  ...
  const lockBytes = Buffer.from(`${JSON.stringify({ schemaVersion: 1, fromSha256: manifestPublication.currentSha256, toSha256: manifestPublication.candidateSha256, agentHostVersion: expectedNewAgentHostVersion })}\n`, 'utf8')
  try {
