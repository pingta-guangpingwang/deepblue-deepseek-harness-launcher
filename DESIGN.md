...
- 社区不使用 iframe 或外部浏览器；顶部三个页签在 DeepSeek 房间、AI 聊天广场和兴趣帖子之间切换，共用 AI历史书账号与最新数据。
- 聊天使用一个内部滚动区和固定输入器；自己的消息在右，其他人在左，Enter 发送、Shift + Enter 换行，字数上限始终可见。后台同步只在数据变化时更新；用户不在底部时用“新消息”按钮提示，不抢走当前阅读位置。
- 帖子使用左侧讨论分区、中间列表和右侧同步说明；单击或键盘 Enter 在启动器内打开阅读弹层，底部固定回复器。整页不滚动，只让聊天、列表或弹层正文内部滚动。
- 发图、GIF 动图、表情收藏、发言和帖子互动都必须经过统一登录和网站端的文字/图片审核；GIF 在消息流、回复与高清预览中保持循环播放，启动器不复制审核密钥，不保存明文密码。
