...
> 模块化发行状态：签名目录 schema 2、多渠道探测与回退、断点下载、SHA-256、受限解包、不可变安装、原子切换与回滚已经落地；真实制品已从空目录安装并启动 Node 24.16.0、Harness 0.1.0-rc.8、pnpm 11.22.0 与 Harness Web。UI 壳同时携带经过同一严格校验的运行模块目录副本；运行目录、UI 壳与三个运行模块均按 Gitee、GitHub、OSS 的顺序自动探测与切换，OSS 只作最后应急兜底。
