...
> 模块化发行状态：签名目录 schema 2、多渠道探测与回退、断点下载、SHA-256、受限解包、不可变安装、原子切换与回滚已经落地；真实制品已从空目录安装并启动 Node 24.16.0、Harness 0.1.0-rc.6、pnpm 11.22.0 与 Harness Web。UI 壳同时携带经过同一严格校验的运行模块目录副本，OSS 目录临时不可读时仍可从已签名的 GitHub 模块完成首次安装。当前大模块只有 GitHub Release 是已验证公开渠道；Gitee 或 OSS 大模块只会在真实文件可匿名下载后加入，避免发布无效地址。
