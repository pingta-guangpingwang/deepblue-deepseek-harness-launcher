...
describe('catalog signature verification', () => {
  ...
  it('keeps the modular catalog on an isolated v2 trust root and endpoint', () => {
    ...
    const windowsBuilder = readFileSync(path.resolve('scripts/build-windows-variants.ps1'), 'utf8')
    const signerSource = readFileSync(path.resolve('scripts/sign-manifest.mjs'), 'utf8')
    const packageJson = JSON.parse(readFileSync(path.resolve('package.json'), 'utf8')) as { scripts: Record<string, string> }
    ...
    expect(signerSource).toContain("process.env.LAUNCHER_SIGNING_KEY_ID || 'runtime-production-v2-1'")
    expect(signerSource).toContain('Signing key does not match resources/runtime-update-public-key.pem')
    expect(packageJson.scripts['release:qa-public-ui-hot-update']).toContain('qa-public-launcher-ui-hot-update.mjs')
  })
})
