...
const config=${behavior};const pet=document.getElementById('pet');const image=document.getElementById('image');const canvas=document.getElementById('canvas');const bubble=document.getElementById('bubble');let bubbleTimer=0,reactionTimer=0,frameTimer=0,atlasState='idle';
...
function react(){speak();atlasState='click';pet.classList.remove('hop','spin','heart');void pet.offsetWidth;pet.classList.add(config.clickMotion||'heart');clearTimeout(reactionTimer);reactionTimer=setTimeout(()=>{atlasState='idle';pet.classList.remove('hop','spin','heart')},760)}
pet.addEventListener('click',react);pet.addEventListener('mouseenter',()=>{if(atlasState==='idle')atlasState='hover'});pet.addEventListener('mouseleave',()=>{if(atlasState==='hover')atlasState='idle'});image.src=${mediaUrl};
${pixelAtlas ? `image.addEventListener('load',()=>{const columns=8;const rows=image.naturalHeight%11===0?11:image.naturalHeight%9===0?9:1;const fw=image.naturalWidth/columns,fh=image.naturalHeight/rows;canvas.width=fw;canvas.height=fh;const ctx=canvas.getContext('2d',{willReadFrequently:true});const scratch=document.createElement('canvas');scratch.width=fw;scratch.height=fh;const scan=scratch.getContext('2d',{willReadFrequently:true});const cache=new Map();const rowFor=()=>Math.min(rows-1,atlasState==='click'?(config.clickMotion==='hop'?4:3):atlasState==='hover'?6:0);const framesFor=row=>{if(cache.has(row))return cache.get(row);const frames=[];const minimum=Math.max(12,Math.floor(fw*fh*.002));for(let frame=0;frame<columns;frame+=1){scan.clearRect(0,0,fw,fh);scan.drawImage(image,frame*fw,row*fh,fw,fh,0,0,fw,fh);const pixels=scan.getImageData(0,0,fw,fh).data;let visible=0;for(let pixel=3;pixel<pixels.length&&visible<minimum;pixel+=4)if(pixels[pixel]>8)visible+=1;if(visible>=minimum)frames.push(frame)}if(!frames.length)frames.push(0);cache.set(row,frames);return frames};let index=0,lastRow=-1;const draw=()=>{const row=rowFor();if(row!==lastRow){index=0;lastRow=row}const frames=framesFor(row);const frame=frames[index%frames.length];ctx.clearRect(0,0,fw,fh);ctx.drawImage(image,frame*fw,row*fh,fw,fh,0,0,fw,fh);canvas.dataset.frameIndex=String(frame);canvas.dataset.animationRow=String(row);index+=1};draw();frameTimer=setInterval(draw,150)});` : ''}
