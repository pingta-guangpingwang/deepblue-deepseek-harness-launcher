...
describe.skipIf(!enabled)('generated runtime modules', () => {
  it('installs the exact release artifacts and boots the packaged Harness CLI and Web service', { timeout: 240_000 }, async () => {
    ...
    const agentHost = generated.modules.find((module) => module.id === 'agent-host')
    if (!harness || !packageManager || !launcherUi || !agentHost) throw new Error('Generated catalog is missing required modules')
    ...
    const hostResult = await store.install(agentHost, generated.modules, 'win32', 'x64')
    expect(JSON.parse(await readFile(path.join(hostResult.root, 'module.json'), 'utf8'))).toMatchObject({ id: 'agent-host', version: agentHost.version, protocol: 1 })
    expect(await readFile(path.join(hostResult.root, 'connector', 'host-child.mjs'), 'utf8')).toContain('runHostChild')
    expect(JSON.parse(await readFile(path.join(hostResult.root, 'node_modules', 'ws', 'package.json'), 'utf8')).name).toBe('ws')
    ...
    expect(harnessVersion.output).toContain('0.1.1-rc.2')
