...
import { ArrowDown, ArrowLeft, Bot, CheckCircle2, CircleAlert, Folder, LoaderCircle, LogIn, MessageSquare, Monitor, Pause, Play, Plus, RefreshCw, Send, Settings2, Square, Unplug } from 'lucide-react'
...
export function AgentWorkspacePage(props: { snapshot: LauncherSnapshot; onLogin(): void; initialSource?: 'local' | 'cloud' | 'groups'; manageRequest?: number; onManageRequestHandled?(request: number): void }): React.JSX.Element {
...
export function CloudAgentWorkspace({ snapshot, onLogin, manageRequest, onManageRequestHandled, initialAgentId = '', initialProjectId = '', initialSessionId = '', detached = false }: { snapshot: LauncherSnapshot; onLogin(): void; manageRequest?: number; onManageRequestHandled?(request: number): void; initialAgentId?: string; initialProjectId?: string; initialSessionId?: string; detached?: boolean }): React.JSX.Element {
  ...
  return <section className="agent-workspace" data-mobile-pane={mobilePane} aria-label="智能体工作台">
    ...
    {host && <div className="aw-management">
      ...
      {host.discovered.length > 0 && <div className="aw-discovered">{host.discovered.map(item => <div key={item.adapter}><span><strong>{item.name}</strong><small>{item.message || (item.available ? '本机已检测到' : '未检测到可用命令')}</small></span><button className="small-button" disabled={Boolean(busy) || !item.available || !host.deviceId} onClick={() => setConfirmAction({ action: 'add_agent', adapter: item.adapter as AgentAdapter, name: item.name })}><Plus size={13} />连接智能体</button></div>)}</div>}
      {confirmAction && <div className="aw-confirm" role="alert"><p>{['add_agent', 'authorize_agent'].includes(confirmAction.action) ? '允许连接这个智能体，自动同步它的全部原生项目与对话目录，并在这些项目内接收远程任务？之后新增项目也自动同步，无需逐个授权。不扫描整个磁盘，不复制登录凭据。' : confirmAction.action === 'revoke_device' ? '解绑会停止本机接收任务并撤销设备凭据，不会删除你的项目文件。' : '移除此托管绑定？原生项目和会话文件不会被删除。'}</p><button className="small-button" onClick={() => setConfirmAction(undefined)}>取消</button><button className="small-button" onClick={() => void hostAction(confirmAction)}>{['add_agent', 'authorize_agent'].includes(confirmAction.action) ? '授权这个智能体' : confirmAction.action === 'revoke_device' ? '确认解绑' : '确认移除'}</button></div>}
    </div>}
