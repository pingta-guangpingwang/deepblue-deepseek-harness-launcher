...
export function AgentWorkspacePage(props: { snapshot: LauncherSnapshot; onLogin(): void; initialSource?: 'local' | 'cloud' | 'groups'; manageRequest?: number; onManageRequestHandled?(request: number): void }): React.JSX.Element {
  ...
  return <div className="agent-workspace"><nav className="aw-toolbar aw-source-tabs" aria-label="工作台数据来源"><button className={source === 'local' ? 'primary-button' : 'small-button'} aria-pressed={source === 'local'} onClick={() => setSource('local')}>本机项目与对话</button><button className={source === 'cloud' ? 'primary-button' : 'small-button'} aria-pressed={source === 'cloud'} onClick={() => setSource('cloud')}>网站同步与托管</button><button className={source === 'groups' ? 'primary-button' : 'small-button'} aria-pressed={source === 'groups'} onClick={() => setSource('groups')}>多智能会话</button></nav><div className="aw-source-content">{source === 'local' ? <LocalAgentWorkspace {...props} /> : source === 'cloud' ? <CloudAgentWorkspace {...props} /> : <AgentSessionGroups snapshot={props.snapshot} onLogin={props.onLogin} />}</div></div>
}
