...
const APPROVAL_ACTION_ID = 'c'.repeat(32)
...
const qaGroupDetails = new Map<string, { group: Record<string, unknown>; roles: Array<Record<string, unknown>>; runs: Array<Record<string, unknown>>; actions: Array<Record<string, unknown>> }>([[GROUP_ID, {
  group: qaGroups[0]!,
  roles: [
    { id: PLAN_ROLE_ID, role_name: '规划', responsibility: '拆解目标并明确验收边界', agent_id: 'qa-codex', agent_name: 'Codex · 网站开发', project_id: 'qa-project', project_name: 'AI历史书网站', native_session_id: 'qa-session', native_session_title: '修复连接并同步原生会话', runtime_status: 'ready', readiness_source: 'host' },
    { id: REVIEW_ROLE_ID, role_name: '复核', responsibility: '检查结果与发布门禁', agent_id: 'qa-claude', agent_name: 'Claude Code', project_id: 'qa-project', project_name: 'AI历史书网站', native_session_id: 'qa-session-2', native_session_title: '检查网页工作台布局', runtime_status: 'offline', readiness_source: 'host', status_message: '请先在本机启动并登录' }
  ],
...
const qaGroupCandidates = {
  agents: host.agents.map(agent => ({ id: agent.id, display_name: agent.name, adapter_code: agent.adapter, runtime_status: agent.runtimeStatus, readiness_source: 'host', status_message: agent.message })),
...
const fixture = {
  ...
  installApprovalRun() { const groupId = '6'.repeat(32); const detail = qaGroupDetails.get(groupId); const target = detail?.roles[1]; if (detail && target) { detail.group.active_run_count = 1; detail.group.active_run_id = APPROVAL_RUN_ID; detail.group.latest_run_status = 'awaiting_approval'; detail.runs.unshift({ id: APPROVAL_RUN_ID, instruction: '复核并发布安全修复', status: 'awaiting_approval', latest_summary: '主控生成了一次委派，等待用户明确允许', client_request_id: 'qa-approval-request', target_role_ids: detail.roles.map(role => role.id), mode: 'coordinator', coordinator_role_id: detail.roles[0]?.id, max_turns: 4, content_available: true, created_at: new Date().toISOString() }); detail.actions.unshift({ id: APPROVAL_ACTION_ID, run_id: APPROVAL_RUN_ID, role_id: target.id, ordinal: 2, action_type: 'delegate', status: 'awaiting_user_approval', instruction: '检查补丁并在授权项目内修正发现的问题', approval_required: true, approval_reason: 'cross_project_write_capable_delegate', created_at: new Date().toISOString() }); bumpGroupRevision(groupId) } },
