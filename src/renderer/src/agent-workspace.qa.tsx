...
const APPROVAL_RUN_ID = 'b'.repeat(32)
const COORDINATOR_APPROVAL_ACTION_ID = 'c'.repeat(32)
const DELEGATE_APPROVAL_ACTION_ID = 'd'.repeat(32)
...
const qaGroupDetails = new Map<string, { group: Record<string, unknown>; roles: Array<Record<string, unknown>>; runs: Array<Record<string, unknown>>; actions: Array<Record<string, unknown>> }>([[GROUP_ID, {
  group: qaGroups[0]!,
  roles: [
    { id: PLAN_ROLE_ID, role_name: '规划', responsibility: '拆解目标并明确验收边界', agent_id: 'qa-codex', agent_name: 'Codex · 网站开发', project_id: 'qa-project', project_name: 'AI历史书网站', native_session_id: 'qa-session', native_session_title: '修复连接并同步原生会话', runtime_status: 'ready', can_dispatch: true, readiness_source: 'host' },
    { id: REVIEW_ROLE_ID, role_name: '复核', responsibility: '检查结果与发布门禁', agent_id: 'qa-claude', agent_name: 'Claude Code', project_id: 'qa-project', project_name: 'AI历史书网站', native_session_id: 'qa-session-2', native_session_title: '检查网页工作台布局', runtime_status: 'offline', can_dispatch: false, readiness_source: 'host', status_message: '请先在本机启动并登录' }
  ],
...
const qaGroupRevisions = new Map<string, number>([[GROUP_ID, 1]])
const groupRevision = (groupId: string): string => (qaGroupRevisions.get(groupId) || 0).toString(16).padStart(64, '0')
const bumpGroupRevision = (groupId: string): void => { qaGroupRevisions.set(groupId, (qaGroupRevisions.get(groupId) || 0) + 1) }
const qaGroupCandidates = {
  agents: host.agents.map(agent => ({ id: agent.id, display_name: agent.name, adapter_code: agent.adapter, runtime_status: agent.runtimeStatus, can_dispatch: agent.runtimeStatus === 'ready' || agent.runtimeStatus === 'busy', readiness_source: 'host', status_message: agent.message })),
...
const fixture = {
  ...
  settleCreatedGroupRun() { const groupId = '6'.repeat(32); const detail = qaGroupDetails.get(groupId); const run = detail?.runs.find(item => item.id === '7'.repeat(32)); if (detail && run) { run.status = 'cancelled'; run.latest_summary = '本机已确认取消'; detail.group.active_run_count = 0; detail.group.active_run_id = null; detail.group.latest_run_status = 'cancelled'; bumpGroupRevision(groupId) } },
  installCoordinatorApprovalRun() { const groupId = '6'.repeat(32); const detail = qaGroupDetails.get(groupId); const coordinator = detail?.roles[0]; if (detail && coordinator) { detail.group.active_run_count = 1; detail.group.active_run_id = APPROVAL_RUN_ID; detail.group.latest_run_status = 'awaiting_approval'; detail.runs.unshift({ id: APPROVAL_RUN_ID, instruction: '复核并发布安全修复', status: 'awaiting_approval', latest_summary: '主控尚未执行，等待用户明确允许', client_request_id: 'qa-approval-request', target_role_ids: detail.roles.map(role => role.id), mode: 'coordinator', coordinator_role_id: coordinator.id, max_turns: 2, content_available: true, created_at: new Date().toISOString() }); detail.actions.unshift({ id: COORDINATOR_APPROVAL_ACTION_ID, run_id: APPROVAL_RUN_ID, role_id: coordinator.id, ordinal: 1, action_type: 'coordinator', status: 'awaiting_user_approval', instruction: '复核并发布安全修复', approval_required: true, approval_reason: 'coordinator_run_write_capable', created_at: new Date().toISOString() }); bumpGroupRevision(groupId) } },
  installDelegateApproval() { const groupId = '6'.repeat(32); const detail = qaGroupDetails.get(groupId); const run = detail?.runs.find(item => item.id === APPROVAL_RUN_ID); const target = detail?.roles[1]; if (detail && run && target) { run.status = 'awaiting_approval'; run.latest_summary = '主控生成了一次委派，等待用户明确允许'; detail.group.latest_run_status = 'awaiting_approval'; detail.actions.unshift({ id: DELEGATE_APPROVAL_ACTION_ID, run_id: APPROVAL_RUN_ID, role_id: target.id, ordinal: 2, action_type: 'delegate', status: 'awaiting_user_approval', instruction: '检查补丁并在授权项目内修正发现的问题', approval_required: true, approval_reason: 'cross_project_write_capable_delegate', created_at: new Date().toISOString() }); bumpGroupRevision(groupId) } },
  installMobileApprovalRun() { const detail = qaGroupDetails.get(GROUP_ID); const run = detail?.runs.find(item => item.id === UNKNOWN_RUN_ID); const action = detail?.actions.find(item => item.id === UNKNOWN_ACTION_ID); if (detail && run && action) { run.status = 'awaiting_approval'; run.latest_summary = '等待用户允许主控委派'; action.status = 'awaiting_user_approval'; action.effective_status = 'awaiting_user_approval'; action.instruction = '在手机确认后检查发布证据'; action.approval_required = true; action.approval_reason = 'cross_project_write_capable_delegate'; delete action.error_code; detail.group.latest_run_status = 'awaiting_approval'; bumpGroupRevision(GROUP_ID) } },
  ...
  agentWorkspaceRequest: async (request: AgentWorkspaceRequest) => {
    ...
    try {
      ...
      if (request.action === 'group_detail') {
        ...
        const detailRevision = groupRevision(groupId)
        if (detail && request.params?.afterRevision === detailRevision) return { ok: true, changed: false, detailRevision }
        return detail ? { ok: true, changed: true, detailRevision, window: { maxRuns: 12, maxActions: 144, runCount: detail.runs.length, actionCount: detail.actions.length, hasMoreRuns: false, hasMoreActions: false, maxRunBodyChars: 8000, maxActionInstructionChars: 4000, maxApprovalInstructionChars: 8000, maxActionResultChars: 8000 }, ...structuredClone(detail) } : { ok: false, message: '会话群不存在' }
      }
      ...
      if (request.action === 'group_delete') {
        ...
        qaGroupRevisions.delete(groupId)
      ...
      if (request.action === 'group_approve') {
        const groupId = String(request.body?.groupId || '')
        const runId = String(request.body?.runId || '')
        const actionId = String(request.body?.actionId || '')
        const detail = qaGroupDetails.get(groupId)
        const run = detail?.runs.find(item => item.id === runId)
        const action = detail?.actions.find(item => item.id === actionId)
        if (!detail || !run || !action || action.status !== 'awaiting_user_approval') return { ok: false, message: '待批准委派不存在或状态已变化' }
        action.status = 'approved'; action.approval_required = false; action.approved_at = new Date().toISOString(); run.status = 'running'; run.latest_summary = '用户已允许本次委派'; detail.group.latest_run_status = 'running'; bumpGroupRevision(groupId)
        return { ok: true, runId, actionId, status: 'running' }
      }
      if (request.action === 'group_cancel') {
        ...
        const detail = qaGroupDetails.get(groupId)
        const run = detail?.runs.find(item => item.id === runId)
        if (run) { run.status = 'cancel_requested'; run.latest_summary = '等待本机停止确认'; if (detail) { detail.group.status = 'active'; detail.group.active_run_count = 1; detail.group.latest_run_status = 'cancel_requested'; const pending = detail.actions.find(item => item.run_id === runId && item.status === 'awaiting_user_approval'); if (pending) pending.status = 'cancelled'; bumpGroupRevision(groupId) } }
