...
const MESSAGE_ONE_ID = 'b'.repeat(32)
const MESSAGE_TWO_ID = 'c'.repeat(32)
const MESSAGE_THREE_ID = 'd'.repeat(32)
const MESSAGE_FOUR_ID = 'f'.repeat(32)
const MESSAGE_FIVE_ID = '0'.repeat(32)
...
const qaRooms: Array<Record<string, unknown>> = [{ contract_version: 2, id: ROOM_ID, name: '产品发布室', coordinator_member_id: COORDINATOR_ID, max_steps: 12, default_access: 'workspace_write', approval_policy: 'bounded_run', definition_revision: 3, state_revision: 12, status: 'active', active_run_id: UNKNOWN_RUN_ID, latest_run_status: 'unknown', updated_at: '2026-09-09T08:40:00+08:00' }]
const qaRoomDetails = new Map<string, RoomFixtureDetail>([[ROOM_ID, {
  ...
  runs: [
    ...
    { id: COMPLETE_RUN_ID, room_id: ROOM_ID, root_message_id: MESSAGE_ONE_ID, routing_kind: 'coordinator', coordinator_member_id: COORDINATOR_ID, target_member_ids: [], definition_revision: 3, max_steps: 12, approval_policy: 'bounded_run', access: 'workspace_write', status: 'completed', step_count: 4, requires_approval: false, approval_id: COMPLETE_APPROVAL_ID, approved_at: '2026-09-09T08:31:30+08:00', final_message_id: MESSAGE_FOUR_ID, content_available: true, created_at: '2026-09-09T08:31:00+08:00', completed_at: '2026-09-09T08:36:00+08:00' }
  ],
  actions: [
  ...
  agentWorkspaceRequest: async (request: AgentWorkspaceRequest) => {
    ...
    try {
      ...
      if (request.action === 'room_create' || request.action === 'room_update') {
        ...
        const room = { contract_version: 2, id: roomId, name: body.name, coordinator_member_id: body.coordinatorMemberId, max_steps: body.maxSteps, default_access: body.defaultAccess, approval_policy: 'bounded_run', definition_revision: definition, state_revision: request.action === 'room_create' ? 0 : Number(qaRoomDetails.get(roomId)?.room.state_revision || 0) + 1, status: 'active', updated_at: new Date().toISOString() }
      ...
      if (request.action === 'room_send') {
        ...
        if (invalidNextRoomSend) {
          const kind = invalidNextRoomSend; invalidNextRoomSend = ''
          const base = { ok: true, contractVersion: 2, messageId: hexId(901), runId: hexId(902), approvalId: hexId(903), status: 'awaiting_approval', requiresApproval: true, replayed: false }
          if (kind === 'missing_version') { const { contractVersion: _removed, ...rest } = base; return rest }
          if (kind === 'wrong_version') return { ...base, contractVersion: 1 }
          if (kind === 'queued' || kind === 'running') return { ...base, status: kind }
          if (kind === 'missing_approval') return { ...base, approvalId: null }
          if (kind === 'bad_ids') return { ...base, messageId: 'bad-message' }
        }
        const body = request.body || {}; const clientRequestId = String(body.clientRequestId || '')
        const replay = roomRequests.get(clientRequestId); if (replay) return { ok: true, contractVersion: 2, ...replay, status: 'awaiting_approval', requiresApproval: true, replayed: true }
