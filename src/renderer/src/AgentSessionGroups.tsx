...
const DETAIL_REVISION_PATTERN = /^[a-f0-9]{64}$/
const RUN_STATUSES = new Set(['queued', 'running', 'awaiting_approval', 'unknown', 'completed', 'failed', 'cancel_requested', 'cancelled'])
...
export function normalizeRoomSummary(value: unknown): AgentRoomSummary {
  ...
  return {
    ...
    coordinatorMemberId,
    maxSteps: exactInteger(field(row, 'max_steps', 'maxSteps'), '房间安全步数', 1, 12),
    defaultAccess: exactWorkspaceAccess(field(row, 'default_access', 'defaultAccess'), '房间权限'),
    approvalPolicy: exactApprovalPolicy(field(row, 'approval_policy', 'approvalPolicy'), '房间'),
    definitionRevision: exactInteger(field(row, 'definition_revision', 'definitionRevision'), '房间定义版本', 1),
    stateRevision: exactInteger(field(row, 'state_revision', 'stateRevision'), '房间状态版本', 0),
    status: 'active',
    activeRunId,
...
function normalizeRun(value: unknown, room: AgentRoomSummary, memberIds: Set<string>): AgentRoomRun {
  const row = object(value)
  ...
  return {
    ...
    definitionRevision,
    maxSteps,
    approvalPolicy: exactApprovalPolicy(field(row, 'approval_policy', 'approvalPolicy'), '任务'),
    status,
    stepCount,
    access: exactWorkspaceAccess(row.access, '任务权限'),
    requiresApproval,
    approvalId,
    approvedAt,
...
function normalizeCandidates(value: unknown): CandidateCatalog {
  ...
  return {
    agents,
    truncated: { agents: bool(truncated.agents), projects: bool(truncated.projects) },
    limits: {
      candidateAgents: exactInteger(limits.candidateAgents, '候选智能体上限', 1),
      projects: exactInteger(limits.projects, '候选项目上限', 1),
      maxMembers: exactInteger(limits.maxMembers, '房间成员上限', 2)
    }
  }
}
...
export function AgentSessionGroups({ snapshot, onLogin }: { snapshot: LauncherSnapshot; onLogin(): void }): React.JSX.Element {
  ...
  const mentionTokensRef = useRef<RoomMentionToken[]>([])
  ...
  function changeDraft(value: string, cursor: number): void {
    const nextTokens = reconcileMentionTokens(draftRef.current, value, mentionTokensRef.current)
    draftRef.current = value; mentionTokensRef.current = nextTokens; setDraft(value); setMentionTokens(nextTokens); detectMention(value, cursor)
  }

  function selectMention(member: AgentRoomMember): void {
    if (!mentionQuery) return
    const currentDraft = draftRef.current
    const currentTokens = mentionTokensRef.current
    const cursor = textarea.current?.selectionStart ?? currentDraft.length
    const label = `@${member.mentionHandle}`
    const next = `${currentDraft.slice(0, mentionQuery.start)}${label} ${currentDraft.slice(cursor)}`
    const reconciled = reconcileMentionTokens(currentDraft, next, currentTokens)
    const token = { memberId: member.id, start: mentionQuery.start, end: mentionQuery.start + label.length, label }
    const nextTokens = [...reconciled.filter(item => item.end <= token.start || item.start >= token.end), token].sort((left, right) => left.start - right.start)
    draftRef.current = next; mentionTokensRef.current = nextTokens; setDraft(next); setMentionTokens(nextTokens); setMentionQuery(undefined)
  ...
  async function send(event?: FormEvent): Promise<void> {
    ...
    const attemptKey = `${targetRoomId}\0${signature}`
    let clientRequestId = sendSubmissions.current.get(attemptKey)
    if (!clientRequestId) {
      if (sendSubmissions.current.size >= 32) sendSubmissions.current.delete(sendSubmissions.current.keys().next().value as string)
      clientRequestId = crypto.randomUUID(); sendSubmissions.current.set(attemptKey, clientRequestId)
    }
