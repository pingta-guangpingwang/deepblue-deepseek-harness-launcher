...
export function LocalAgentWorkspace({ snapshot, onLogin }: { snapshot: LauncherSnapshot; onLogin(): void }): React.JSX.Element {
  ...
  const nativeOwned = desktopSession && !host?.desktopRelay?.ready
  const messages = host?.localHistory?.sessionId === sessionId ? host.localHistory.messages : []
  const tasks = host?.localTasks?.filter(item => item.projectId === projectId && (item.sessionId || '') === sessionId) || []
  ...
  useEffect(() => { if (!projects.some(item => item.id === projectId)) { setProject(projects[0]?.id || ''); setSession(host?.localCatalog?.sessions.find(item => item.projectId === projects[0]?.id)?.id || '') } }, [host?.localCatalog, adapter])
  useEffect(() => {
    if (!sessionId) return
    let stopped = false
    let timer: ReturnType<typeof setTimeout>
  ...
  function selectProject(id: string): void { setProject(id); setSession(host?.localCatalog?.sessions.find(item => item.projectId === id)?.id || ''); setDraft(''); setModel(''); stick.current = true }
  ...
  async function send(): Promise<void> {
    ...
    const signature = JSON.stringify({ projectId, sessionId, draft, model, fileIds })
    if (requestId.current?.signature !== signature) requestId.current = { signature, id: crypto.randomUUID() }
    setBusy(true); setError('')
    try {
      const result = await action({ action: 'send_local', projectId, sessionId: sessionId || undefined, instruction: draft.trim(), model: model || undefined, fileIds, requestId: requestId.current.id })
  ...
  return <section className="agent-workspace" aria-label="本机智能体项目">
    ...
    <div className="aw-panes">
      ...
      <main className="aw-conversation-pane"><header className="aw-conversation-heading"><div><h2>{session?.title || '本机新会话'}</h2><p className="aw-local-path">{project?.path || '最新活动的项目优先显示'}</p></div><button className="small-button" disabled={!project || busy} onClick={() => void sync()}>{binding ? '立即同步网站' : '同步此项目到网站'}</button></header>
        <div className="aw-messages" ref={scroll} tabIndex={0} aria-label="本机原生对话" onScroll={() => { const node = scroll.current; if (node) stick.current = node.scrollHeight - node.scrollTop - node.clientHeight < 64 }}>{messages.map((message, index) => <article className={`aw-message ${message.role}`} key={`${message.id}:${index}`}><div className="aw-message-byline"><strong>{message.role === 'user' ? '你' : names[project?.adapter || ''] || '智能体'}</strong><time>{stamp(message.occurredAt)}</time></div><div className="aw-message-body">{message.text}</div></article>)}{tasks.map(task => <article className="aw-message assistant" key={task.id}><div className="aw-message-byline"><strong>本机任务 · {task.status === 'running' ? '执行中' : task.status === 'delivered' ? '已送达桌面' : task.status === 'unconfirmed' ? '结果未确认' : task.status === 'completed' ? '完成' : task.status === 'cancelled' ? '已停止' : '失败'}</strong></div><p>{task.instruction}</p><div className="aw-message-body">{task.reply || task.summary}</div>{task.status === 'running' && task.backend !== 'desktop' && <button className="small-button" onClick={() => void action({ action: 'cancel_local', taskId: task.id }).catch(cause => setError(String(cause.message)))}>停止本机任务</button>}</article>)}{!messages.length && !tasks.length && <p className="aw-inline-empty">{sessionId ? '正在读取本机对话…' : '可以直接发送任务，创建原生会话。'}</p>}</div>
        <form className="aw-composer" onSubmit={event => { event.preventDefault(); void send() }}><div className="aw-toolbar"><button type="button" className="small-button" disabled={busy || running} onClick={() => void chooseFiles()}><Paperclip size={14} />添加本机文件</button><select onFocus={() => void refreshModels()} aria-label="本机智能体模型" value={model} onChange={event => setModel(event.target.value)}><option value="">智能体默认模型</option>{host?.localCatalog?.models?.filter(item => item.adapter === project?.adapter).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button type="button" className="small-button" onClick={() => void refreshModels()}>刷新模型</button><button type="button" className="small-button" disabled={!project || running} onClick={() => { setSession(''); stick.current = true }}>新建对话</button></div>{fileIds.length > 0 && <div className="aw-local-files">{host?.localFiles?.filter(file => fileIds.includes(file.id)).map(file => <button type="button" className="small-button" key={file.id} onClick={() => setFileIds(ids => ids.filter(id => id !== file.id))}>{file.name} · 移除</button>)}</div>}<textarea aria-label="本机任务内容" placeholder="直接交给本机智能体；无需网站登录或签到" value={draft} maxLength={12000} disabled={!project || busy || running} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void send() } }} /><div className="aw-composer-footer"><span>音视频作为文件交付；具体解析能力取决于智能体。</span><button type="submit" className="primary-button" disabled={!project || !draft.trim() || busy || running || nativeOwned}><Send size={14} />{nativeOwned ? '桌面桥接未就绪' : busy ? '提交中…' : desktopSession ? '发送到桌面原对话' : '本机发送'}</button></div></form>
