...
2026-09-09。状态：本地纵向候选及安全复核已完成，尚未执行生产迁移、安装 Worker 或公开发布。
...
- 可选“主控协调”：选择现有角色作为主控，预览参与成员、授权项目与最大执行轮数后开始；主控不能添加新成员或扩大权限。由于现有智能体没有可强制验证的只读模式，初始主控任务必须先展示主控项目/会话、原始任务和本轮上限，由用户点“允许主控开始”；主控之后生成的每一次委派还要展示目标角色、项目、原生会话和完整任务，由用户点“允许委派”后才能创建真实任务。
...
5. 状态明确分为 queued、running、awaiting_approval、unknown、completed、failed、cancel_requested、cancelled。初始主控动作和协调者生成的委派提案用 `awaiting_user_approval`；用户确认后先进入 `approved`，Worker 再以事务 fence 转为 `dispatching`。批准前不得创建底层 task/command。仅本机完成回执可标记 completed；进度不能代替结果。
...
- `group_approve`：POST，正文严格为 `{ groupId, runId, actionId }`；只允许当前用户确认当前 run 中仍为 `awaiting_user_approval` 的初始主控动作或单次委派。初始批准仅覆盖该主控在本轮上限内的规划/汇总范围，不覆盖其他角色；拒绝不新增动作，复用 `group_cancel` 停止本轮。
