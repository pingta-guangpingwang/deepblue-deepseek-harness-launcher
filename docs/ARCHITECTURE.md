...
│  ├─ launcher-ui/<hash>       可独立热更新的前端界面模块
...
| `harness-core` 0.1.1-rc.2 | 构建后签名清单为准 | 首次安装 Harness |
...
签名目录 schema 2 对模块 ID、平台、架构、压缩格式、下载与解压大小、SHA-256、依赖关系、镜像主机、路径前缀和可执行探针做闭集校验。启动时只比较已经安装的模块；发现版本差异后先列出模块、旧/新版本与总下载量，用户确认后才下载。每个模块固定按 Gitee、OSS、GitHub 顺序逐条探测：Gitee 不可用便立即尝试 OSS，不会先等待 GitHub；任一线路持续 15 秒没有新增下载字节、下载失败或校验失败都会自动切换下一条。任务状态同时呈现各模块的渠道、阶段、真实字节以及按模块大小加权的总进度。支持跨镜像续传，但最终仍重新做完整摘要校验。压缩包只允许相对文件/目录，不允许绝对路径、`..`、符号链接或其他特殊条目。安装先进入 `.staging`，单模块探针通过后原子改名并更新 `state.json`；任一后续模块失败时按相反顺序恢复本轮已经切换的模块，整批成功后自动重启启动器。
...
从 0.10.23 起，UI 壳进一步分成长期稳定的 Electron 基础内核与 `launcher-ui` 前端模块。基础内核只负责窗口、安全协议、白名单 IPC、签名目录、模块原子安装和回滚；页面、布局、目录呈现与仅依赖既有 IPC 的交互打成约数百 KiB 的内容寻址模块。启动时优先读取 `runtime/modules/launcher-ui/<hash>/renderer/index.html`，入口或本地资源不完整时自动回退上一版，再回退随内核附带的界面。纯 UI 更新安装后只刷新当前窗口，不停止正在运行的 Harness，也不重新下载 Electron。

基础内核版本与 UI 模块版本独立：`launcherVersion` 采用语义版本，`launcherUiVersion` 使用渲染目录树 SHA-256 的前 16 位。发布相同内容不会产生新版本；任何字节变化都会得到新目录和新下载文件。需要新增 IPC、改变 Electron 权限或升级原生依赖时才发布新的基础内核，这是少数仍需替换网页安装器的情况。

Node、Harness 核心和包管理器也使用“模块 ID + 模块版本 + 产物哈希”的永久标签，不再借用启动器版本号。只要模块字节没变，其 Gitee、OSS、GitHub 地址就保持不变，无需随基础内核重复上传。
...
- Gitee：国内首选的 `runtime-assets` 固定资源分支，按版本保存 UI 壳与内容寻址运行模块附件；
- OSS：国内第二线路，同时托管不到 1 MB 的联网引导器、内容寻址模块和 schema-2 签名清单；
- GitHub：源码与最终备用的 Release 附件；
...
兴趣社区的 Renderer 不直接持有 Bearer Token，只能通过 Preload 白名单中的 `communityRequest` 调用固定的论坛和聊天 API。Main 进程对 scope、method、action、频道、图片类型与 5 MB 大小上限再次校验，公开 GET 可匿名读取，POST 必须使用当前 AI历史书短期会话。聊天每 12 秒在后台轻量同步，只在消息签名变化时替换列表，最多显示最新 100 条。
