...
签名目录 schema 2 对模块 ID、平台、架构、压缩格式、下载与解压大小、SHA-256、依赖关系、镜像主机、路径前缀和可执行探针做闭集校验。当前下载顺序为 GitHub Releases、OSS 应急兜底；Gitee仓库真实可用后才会加入。支持断点续传，但跨镜像仍重新做完整摘要校验。压缩包只允许相对文件/目录，不允许绝对路径、`..`、符号链接或其他特殊条目。安装先进入 `.staging`，探针通过后才原子改名并更新 `state.json`；失败不会覆盖活动版本。
